...
function contextGreeting() {
  ...
  // Hint in Seitensprache
  const greetLang = getClippyLang();
  const hintMap = {
    de: hint,
    en: hint ? hint.replace('lade einfach deine STL hoch, ich schätze den Preis', 'just upload your STL, I'll estimate the price').replace('Schick mir deine SVG/DXF', 'Send me your SVG/DXF').replace('ich helfe dir beim Material wählen oder Preise einschätzen', 'I'll help you choose materials or estimate prices').replace('Ich erkläre dir den Unterschied zwischen allem', 'I'll explain the difference between everything').replace('Wenn du hier keine Antwort findest, frag mich direkt', 'If you don't find an answer here, just ask me') : null,
  };
  const localHint = hintMap[greetLang] || hint;
  if (localHint) {
    setTimeout(() => appendBot(localHint), 600);
  }
}
  ...
  if (name.endsWith('.stl') || name.endsWith('.obj')) {
    ...
    reader.onload = e => {
      try {
        ...
        typing.remove();
        const errMsg1 = {de:'Hmm, die Datei konnte ich nicht lesen 🤔 Bitte STL im Binary-Format exportieren!', en:'Hmm, couldn't read the file 🤔 Please export STL in binary format!', fr:'Hmm, je n'ai pas pu lire le fichier 🤔 Veuillez exporter le STL en format binaire !', es:'Hmm, no pude leer el archivo 🤔 ¡Por favor exporta el STL en formato binario!', it:'Hmm, non riesco a leggere il file 🤔 Esporta l'STL in formato binario!'}[getClippyLang()] || 'Hmm, die Datei konnte ich nicht lesen 🤔';
        appendBot(errMsg1);
      }
    };
    ...
    typing.remove();
    const fmtErr = {de:`📎 Format <strong>.${name.split('.').pop()}</strong> kenne ich noch nicht. Schick mir STL, OBJ, SVG oder DXF!`, en:`📎 Format <strong>.${name.split('.').pop()}</strong> not supported. Send me STL, OBJ, SVG or DXF!`, fr:`📎 Format <strong>.${name.split('.').pop()}</strong> non reconnu. Envoyez-moi un STL, OBJ, SVG ou DXF !`, es:`📎 Formato <strong>.${name.split('.').pop()}</strong> no compatible. ¡Envíame STL, OBJ, SVG o DXF!`, it:`📎 Formato <strong>.${name.split('.').pop()}</strong> non supportato. Mandami STL, OBJ, SVG o DXF!`}[getClippyLang()] || '';
    appendBot(fmtErr);
  }
}
...
const FALLBACK_LANG = {
  ...
  en: {
    ...
    kosten:   'Prices from ~€5 (PLA). Upload your STL — I'll estimate instantly! 📎',
    ...
    witzig:   ['I'm a paperclip AI. Been here since 1997. 📎', 'Great question! Probably the best one today. 😄', 'If this goes wrong — just ask again. I forget everything anyway. 😅'],
  ...
  fr: {
    ...
    kosten:   'Prix à partir de ~€5 (PLA). Téléchargez votre STL — j'estime immédiatement ! 📎',
    ...
    witzig:   ['Je suis une IA trombone. Depuis 1997. 📎', 'Bonne question ! Probablement la meilleure aujourd'hui. 😄', 'Si ça rate — redemandez. J'oublie tout de toute façon. 😅'],
  ...
  es: {
    ...
    kosten:   'Precios desde ~€5 (PLA). ¡Sube tu STL — estimo al instante! 📎',
  ...
  it: {
    ...
    kosten:   'Prezzi da ~€5 (PLA). Carica il tuo STL — stimo subito! 📎',
